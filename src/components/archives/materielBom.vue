<template>
  ...
  >
    <el-form ref="form" :rules="rules" label-width="80px" label-position="left">
      ...
      <el-form-item label="数量" prop="Number" label-position="left">
        <el-input type="Number" placeholder="请输入数量" v-model="formData.Number"></el-input>
      </el-form-item>
      <el-form-item label="单位" prop="UnitName" label-position="left">
...
export default {
  ...
  data() {
    return {
      ...
      //    rules: {
      //   MaterielName: [
      //     { required: true, message: '请选择或者输入物料名称', trigger: 'change' },
      //   ],
      //   MaterielNo: [
  ...
  computed: {
    ...mapGetters(["GetMaterielsByKeyWord", "GetUnitConvertsByMaterielId"]),
  },
  watch: {
