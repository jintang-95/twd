...
export default {
  ...
  computed: {
    ...mapGetters(["ULFB", "ULFE", "imgUrl", "QuerySupplier", "SaveBrand","GetMateriels"]),
  },
  watch: {
  ...
  methods: {
    ...
    addTable(row) {
      ...
      console.log(row);
      this.form.ReplaceMaterielDetail.push(row);
    ...
    submitData() {
      this.$refs.form.validate((vaild) => {
        if (vaild) {
          ...
          this.form.SType = this.SType;
          this.form.Grade = "0";
          this.form.ManageStyle = 0;
          //console.log(this.form.SType)
          // this.$emit("addMateria", this.form);
          this.addMsg(this.form)
          // this.form = {}
        } else {
          return false;
        }
