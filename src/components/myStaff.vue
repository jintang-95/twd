    ...
    <div class="selectL_r" :class="!labelShow ? 'selectL_radious' : ''">
      ...
      >
        <div slot="reference" class="selectL_r_input">
          <el-input placeholder="请选择" v-model="form.keyValue" clearable readonly></el-input>
              ...
              <div class="tab_connect">
                ...
                <div class="tab_connect_r">
                  <div class="formTop" v-if="isMultipleChoice">
                    <button
                      class="el-button el-button--primary choseCheckbox"
                      size="small"
                      @click="getUser"
                    >确定</button>
                  </div>
                  <el-input
                    ...
                    @keyup.native.enter="searchUserBtn()"
                  ></el-input>
                  <el-cascader
                  ...
                  <div class="userList-scorll" v-if="!isMultipleChoice">
                    <el-radio-group
                      v-model="stuffId_"
                      class="userList"
                      v-for="(item, index) in selectList.userList"
                      :key="index"
                    >
                      <el-radio
                        class="userList_item"
                ...
                <div class="tab_connect_r">
                  ...
                  <div class="userList-scorll" v-if="!isMultipleChoice">
                    <el-radio-group
                      v-model="stuffId_"
                      class="userList"
                      v-for="(item, index) in selectList.jobsList"
                      :key="index"
                    >
                      <el-radio
                        class="userList_item"
...
export default {
  ...
  watch: {
    "$store.state.Humanmanage.QueryJobs": function (data) {
      this.selectList.jobsList = data.ReturnData.Data;
      if (this.QueryStaffFrom.OrganizationId == "") {
        this.allData = data.ReturnData.Data;
        if (this.QueryStaffFrom.OrganizationId == "") {
          this.allData = data.ReturnData.Data;
          if (this.stuffId != "") {
            for (var i = 0; i < this.allData.length; i++) {
              if (this.allData[i].Id == this.stuffId) {
    ...
    "$store.state.Humanmanage.QueryStaff": function (data) {
      this.selectList.userList = data.ReturnData.Data;
      if (this.QueryStaffFrom.OrganizationId == "") {
        this.allData = data.ReturnData.Data;
        if (this.stuffId != "") {
          for (var i = 0; i < this.allData.length; i++) {
