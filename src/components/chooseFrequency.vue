<template>
	...
	checkedList：[] 选中班次ID集合  多选可通过父级传值默认选中 ["1","2""3"]
	SupplierId：string 选中班次ID  单选可通过父级传值默认选中   "1"
	@getToFather //获取数据
 -->
	<div class="selectL" style="padding: 0;">
		<div class="selectL_r">
			<el-popover v-model="tabs.popoverShow" placement="bottom-end" :width="popverWidth" trigger="click">
				<div slot="reference" class="selectL_r_input">
					<span style="color: #0060A0;cursor: pointer;">修改班次</span>
				...
				<div class="seletHome">
					<div class="tab_connect">
						<div class="tab_connect_r">
							...
							<el-input placeholder="请输入内容" prefix-icon="el-icon-search" v-model="tabConnect.searchSupplier"
							 @keyup.native.enter="searchSupplierBtn()">
							</el-input>
	...
	export default {
		...
		watch: {
			"$store.state.Humanmanage.GetFrequencyPages": function(data) {
				this.selectList.supplierList = JSON.parse(JSON.stringify(data.Data));
				this.allData = JSON.parse(JSON.stringify(data.Data));
				if (this.SupplierId != "") {
					this.SupplierId_ = this.SupplierId;
					for (var i = 0; i < this.allData.length; i++) {
						if (this.allData[i].ID == this.SupplierId) {
							this.form.keyValue = this.allData[i].Name;
						}
					}
				}
				if (this.checkedList.length > 0) {
					this.checkedList_ = this.checkedList;
					this.form.keyValue = this.checkedList.length;
				}
			},
			'tabs.acitve': {
				deep: true,
