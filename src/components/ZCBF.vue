...
<template>
  <el-dialog class="ZCBF" :title="title" width="800px" @open="openEvent" :visible.sync="dialogZCBF">
...
export default {
  ...
  watch: {
    ...
    AssetScrapUpdateStatus(newVal, oldVal) {
      // this.dialogZCBF = false
      if (newVal.Code == 0) {
      } else {
        this.$message.error(newVal.Message);
    ...
    AssetScrapGetByNo(newVal, oldVal) {
      if (newVal.Code == 0) {
          this.formData=newVal.ReturnData.Data
          this.formData.Name=newVal.ReturnData.Data.AssetScrapInfos[0].Name
          this.formData.Num=newVal.ReturnData.Data.AssetScrapInfos[0].Num
          this.formData.Addtime=newVal.ReturnData.Data.applyDate
          this.formData.Abstracts=newVal.ReturnData.Data.Remarks
        ...
        this.currentApplyId = newVal.ReturnData.Data.Agent
        ...
        this.title = newVal.ReturnData.Data.AgentName + '的资产报废审批'
  ...
  methods: {
    ...
    getFormDetail() {
      let json = {
        MN: "AssetScrapGetByNo",
        SN: "Assets",
        RequestId: 1,
        UserId: JSON.parse(sessionStorage.getItem("UserDetial")).Id,
        DataContent: {
          ScrapNo: this.BillNum,
        },
      };
      this.MIX_send(json);
    },
