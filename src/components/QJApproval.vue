<template>
  ...
  >
    ...
    <div slot="footer" class="dialog-footer" v-if="noSpCenter">
      ...
      <el-button
        @click="clickCancel"
        v-if="formData.Status != '2' && formData.Status != '4' && formData.Status != '3' && currentUserStaffId == currentApplyId"
      >撤销</el-button>
      <el-button
        @click="clickReject"
        v-if="formData.Status == '1' &&  currentNeedSpId == currentUserStaffId"
  ...
  watch: {
    ...
    GetLeaveByNo (newVal, oldVal) {
      if (newVal.Code == 0) {
        ...
        this.formData.Status = newVal.ReturnData.Data.Status
      }
    }
  },
  props: {
