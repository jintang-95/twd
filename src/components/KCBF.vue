<template>
  <el-dialog class="KCBF" :title="title" width="800px" @open="openEvent" :visible.sync="dialogKCBF">
    ...
    font-size: 16px;"
      >| 物料报废明细</div>
      <div class="top" v-if="top==true"><div @click="showTabsFun"><div>展开</div>  <div class="topSon"> > </div></div></div>
      <el-table v-if="showTabs==true"
        :data="formData.UselessApplyDetail"
        :header-cell-style="{background:'#F0F7FB',color:'#5C87A4'}"
        tooltip-effect="dark"
        fit
      >
        <el-table-column align="center" prop="MaterielId" label="物料编码"></el-table-column>
        <el-table-column align="center" prop="MaterielName" label="物料名称"></el-table-column>
        <el-table-column align="center" prop="MaterielSpec" label="规格"></el-table-column>
        <el-table-column align="center" prop="Number" label="数量"></el-table-column>
      </el-table>
    </div>
          </div>
          <div class="footer" v-if="footer==true"><div @click="visTabsFun"><div>收起</div> <div class="footerSon"> > </div></div></div>
        </div>
      </div>
      <div class="node_info">
...
export default {
  ...
  watch: {
    ...
    GetUselessApplyByNo(newVal, oldVal) {
      console.log(JSON.stringify(newVal));
      if (newVal.Code == 0) {
        console.log(JSON.parse(JSON.stringify(newVal.ReturnData)));
        // this.formData.UselessApplyDetail=newVal.ReturnData.UselessApplyDetail
        // console.log(this.formData.UselessApplyDetail)
        this.$set(this.formData, "UselessApplyDetail", newVal.ReturnData.UselessApplyDetail);
        this.formId=newVal.ReturnData.Id
        // this.formData.UselessCount = newVal.ReturnData.Data.LeaveDayCount
    ...
    ApprovalProcessOrderRecordLogGet(newVal, oldVal) {
      ...
      if (newVal.Code == 0) {
        ...
        this.currentNeedSpId = this.nodeData.filter(
          (item) => item.Status == "1"
        )[0].StaffID;
    ...
    UpdateUselessApplyStatusById(newVal, oldVal) {
      // this.dialogKCBF = false
      if (newVal.Code == 0) {
      } else {
        this.$message.error(newVal.Message);
  ...
  methods: {
    ...
    updateStatusKCBF() {
      let json = {
        MN: "UpdateUselessApplyStatusById ",
        SN: "Useless",
        RequestId: 1,
        UserId: 1,
        DataContent: {
