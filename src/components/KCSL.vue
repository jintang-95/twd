<template>
  <el-dialog class="KCSL" :title="title" width="800px" @open="openEvent" :visible.sync="dialogKCSL">
...
export default {
  ...
  watch: {
    ...
    UpdateApplyStatusById(newVal, oldVal) {
      // this.dialogKCSL = false
      if (newVal.Code == 0) {
      } else {
        this.$message.error(newVal.Message);
    ...
    GetApplyByNo(newVal, oldVal) {
      console.log(JSON.parse(JSON.stringify(newVal.ReturnData)));
      if (newVal.Code == 0) {
        console.log(newVal);
        this.formId=newVal.ReturnData.Id
        //this.formData.UselessApplyDetail=newVal.ReturnData.ApplyDetail
        this.$set(this.formData, "UselessApplyDetail", newVal.ReturnData.ApplyDetail);
