  ...
  >
    <div>
      ...
      >
        ...
        <el-table-column align="center" prop="StandardUnitId" label="标准单位"></el-table-column>
...
export default {
  ...
  watch: {
    typeList(newval,oldval){
      this.typeListJson=JSON.parse(JSON.stringify(newval))
    if(this.typeListJson.length != 0){
      if (this.typeListJson[0].Id != 0) {
        let obj = {
          Id: "0",
          Name: "全部",
        };
        this.typeListJson.unshift(obj);
      }
    }
    },
  ...
  async created() {
    // if(this.materielTable.length==0){
    //   this.$message("暂无数据")
    // }
    this.valueType = this.typeNum;
    await this.getTableData();
    this.typeListJson=JSON.parse(JSON.stringify(this.typeList))
    if(this.typeListJson.length != 0){
      if (this.typeListJson[0].Id != 0) {
        let obj = {
          Id: "0",
