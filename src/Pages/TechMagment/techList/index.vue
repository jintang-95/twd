...
import Dialog from "../dialog/techInfo.vue";
...
export default {
  ...
  computed: {
    ...mapGetters([
      "GetListTechnologyTypeTree",
      "GetListTechnologyInfo",
      "DelTechnologyInfo",
      "SaveTechnologyInfo",
      "GetSingleTechnologyInfo",
    ]),
  },
  watch: {
    GetListTechnologyTypeTree: {
    ...
    GetListTechnologyInfo:{
      handler(newValue, oldValue) {
       if (newValue.Code === 0) {
		   for (var i=0;i< newValue.ReturnData.Data.length;i++){
		   	switch(newValue.ReturnData.Data[i].CycleUnit){
				  case "0":
				  newValue.ReturnData.Data[i].CycleUnit_ = "无";
				  break;
				  case "1":
				  newValue.ReturnData.Data[i].CycleUnit_ = "年";
				  break;
				  case "2":
				  newValue.ReturnData.Data[i].CycleUnit_ = "月";
				  break;
				  case "3":
				  newValue.ReturnData.Data[i].CycleUnit_ = "天";
				  break;
		   	}
			if(newValue.ReturnData.Data[i].DeviceId=="0"){
				newValue.ReturnData.Data[i].DeviceName_ ="未关联设备"
			}
			else{
				newValue.ReturnData.Data[i].DeviceName_  = newValue.ReturnData.Data[i].DeviceName;
			}
		   }
    ...
    async DelTechnologyInfo(newValue, oldValue) {
      if (newValue.Code === 0) {
        this.$message({
          showClose: true,
          message: "操作成功",
    ...
    async SaveTechnologyInfo(newValue, oldValue) {
      if (newValue.Code === 0) {
        this.$message({
          showClose: true,
          message: "保存成功",
