          ...
          <template v-for="(item,index) in columns">
            ...
            >
              <template slot-scope="{row}">
                ...
                <div v-if="row.Status=='待提交审批' ||row.Status== 0">待提交审批</div>
                <div v-if="row.Status=='已取消' ||row.Status== 4">已取消</div>
                <div v-if="row.Status=='已报废' ||row.Status== 5">已报废</div>
                <div v-if="row.Status=='待审批' ||row.Status== 1">待审批</div>
...
export default {
  ...
  watch: {
    ...
    ApprovalProcessOrderRecordCreate(newval, oldval) {
      if (newval.Code == 0) {
        ...
        this.UpdateUselessApplyStatusById(this.sObj);
        this.sObj = {};
        this.GetUselessApplys();
      } else {
        this.$message({
          type: "error",
          message: "提交失败",
    ...
    UselessApplys: function (newval, oldval) {
    ...
    removeUselessApply: function (newval, oldval) {
      if (newval.Code == 0) {
        this.$message({
          type: "success",
          message: "删除成功",
    ...
    saveUselessApply: function (newval, oldval) {
      if (newval.Code == 0) {
        this.$message({
          type: "success",
          message: "提交成功",
    ...
    UselessApplyById: function (newval, oldval) {
      this.detailedTable = newval.ReturnData.UselessApplyDetail || [];
      ...
      this.tabledata01 = JSON.parse(JSON.stringify(this.detailedTable));
  ...
  methods: {
    ...
    // 改变状态
    UpdateUselessApplyStatusById(obj) {
      let json = {
    ...
    // 表格查看事件的回调
    check(val) {
      // val.Status=this.Status[Number(val.Status)]
    ...
    output(val) {
      ...
      obj.Id = val.Id;
      obj.SourceTable = "4";
      this.GetUselessApplyById(val.Id);
      this.popVal = obj;
    },
    //编辑修改
    edit(val) {
    ...
    Commit() {
      this.$refs.PopForm.$refs.ruleForm.validate((valid) => {
        if (valid) {
          let value = JSON.parse(JSON.stringify(this.$refs.PopForm.value));
          value.UselessApplyDetail = this.tabledata01;
          value.TableType = 1;
          this.SaveUselessApply(value);
          this.PopFormInit();
        } else {
          return false;
        }
    ...
    // 删除入库单
    RemoveUselessApply(val) {
      let json = {
        MN: "RemoveUselessApply",
        SN: "Useless",
        DataContent: {
          Ids: val.Id,
        },
    ...
    // 获取物料明细
    GetUselessApplyById(id) {
      let json = {
        MN: "GetUselessApplyById",
        SN: "Useless",
        DataContent: {
          Id: id,
        },
  ...
  created() {
    ...
    this.fromData = fromdata.fromdata06;
    this.fromData01 = fromdata.fromdata07;
    this.columns = show01().columns05;
    this.GetUselessApplys();
    this.GetWarehousesTree();
    this.getTableData();
    this.getTreeData();
    this.outPutFromData = fromdata.fromdata4;
    //this.ApprovalProcessOrderRecordCreate()
  },
  //生命周期 - 挂载完成（访问DOM元素）
  mounted() {},
};
