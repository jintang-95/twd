<template>
  <div class="materiaType">
      ...
      <AddWrite :visible="showMask" :list="typeList" :type="formType" :formData="formData" @closeMask="closeMask" @addMateria="addMateria"></AddWrite>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    GetInventoryById(newValue, oldValue) {
      if (newValue.Code === 0) {
        this.formData.AssetsInventoryDetail = newValue.ReturnData.hasOwnProperty('AssetsInventoryDetail') ? newValue.ReturnData.AssetsInventoryDetail : []
        this.openEdit()
      } else {
        // 失败
      }
